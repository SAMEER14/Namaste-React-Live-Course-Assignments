...
// restaurant list object from swiggy api
const resObj = [
    {
      "type": "restaurant",
      "data": {
...
const BodyComponent = () =>{
    return(
        ...
        {/* Doing the .map of resObj for each object inside the list i.e restaurant we pass and returning the piece of JSX*/}
        {/* WE made config driven UI , data changes -> no of restaurant list changes 
        important point -> pass key also*/}
        </div>
        </div>
        </>
    );
...
// We should make UI => config driven UI
// UI controlled from backend-> config -> data coming from backend
// for dynamic images we put in cdn network

//BEST PRACTICES for writing our code => CODE REVIEWING =>
/* 
=>we will destructure it more, for ex. resData.data.image , resData.data.name , resData.data.___  => again & again not looking good.
=>When passing data as props when calling "RestaurantCard" -> 
<RestaurantCard resDataList = {resObj[0]}/>
<RestaurantCard resDataList = {resObj[1]}/>
.........
this is again repeating the code we can also use the best practice here
we can loop over the array -> the obj list and show it dynamically ,  in JS we use map method (we can use for loop also but best practice is use map function)
=> in functional programming we use MAP, FILTER, REDUCE <= these are best ways
=> inside the res-container 
        {
          resObj.map(restaurant => 
          <RestaurantCard resDataList={restaurant}/>
          )
        }
     Doing the .map of resObj for each object inside the list i.e restaurant we as pass and returning the piece of JSX
    WE made config driven UI , data changes -> no of restaurant list changes 

=>  important point -> pass key also -> passing key is important , when using map function , otherwise it will give warning
<div className="res-container">
        {
          resObj.map(restaurant => 
          <RestaurantCard key={restaurant.data.id} resDataList={restaurant}/>
...
=> why giving index is good ? 
 {
          resObj.map(restaurant,index => 
          <RestaurantCard key={index} resDataList={restaurant}/>
          )
        }
-> React official document states that using index is not a good habit as order of items may change => read the document 

=> not using key <<<<<< "index" key(ok way, use when there is no id present) << unique "key"(best practice)

=> WHY GIVING KEY IS IMPORTANT ?
*/
